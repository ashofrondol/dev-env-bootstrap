...
### bootstrap으로 만들지 않은 외부 프로젝트 분석 (`TARGET=<경로>`)

분석 3종은 대상에 **어떤 파일도 요구하지 않으므로**(uvx로 독립 실행) bootstrap 밖에서 만든
프로젝트도 그대로 검사할 수 있다. `TARGET`에 경로(절대/상대 모두 가능)를 주면 된다:

```bash
# 예: 형제 폴더의 codyssey_B2-1 (bootstrap 저장소 기준 상대경로)
make diagram TARGET=../../codyssey_B2-1
make metrics TARGET=../../codyssey_B2-1 SRC=src   # 패키지가 src/ 아래면 SRC로 좁히기
make arch    TARGET=../../codyssey_B2-1           # 최초 1회 그 폴더에서 `uvx tach init`

# 절대경로도 가능 (WSL에서 Windows 쪽 경로는 /mnt/c/... 형태)
make metrics TARGET=/mnt/c/Users/<사용자>/Desktop/codyssey_B2-1
```

> **`make report`(런타임 프로파일)만은 예외다.** 프로젝트에 내장된 `perf.py` 계측 하네스와
> uv 환경이 필요하므로 **bootstrap으로 만든 프로젝트에서만** 동작한다. 외부 프로젝트의 품질은
> 위 정적 3종(diagram/metrics/arch)으로 보고, 굳이 런타임 프로파일이 필요하면
> `cp templates/python/perf.py <대상>/ && (cd <대상> && uv add --dev psutil)` 후 실행한다.
